    ...
    <Card className="hover-elevate min-w-0 border-l-2 border-l-primary/60" data-testid={`card-metric-${title.toLowerCase().replace(/\s+/g, "-")}`}>
      ...
      <CardContent>
        <div className="space-y-1 min-w-0">
          <div className={`text-xl sm:text-2xl lg:text-3xl font-bold truncate ${isNumeric ? 'font-mono tabular-nums' : 'font-heading'}`} data-testid={`text-value-${title.toLowerCase().replace(/\s+/g, "-")}`}>
            {value}
          </div>
          {description && (
